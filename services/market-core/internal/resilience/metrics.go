...
func NewMetrics() *Metrics {
	return &Metrics{snapshot: Snapshot{ActiveAuthorities: map[string]string{}, Provider: map[string]ProviderSnapshot{}}}
}
...
func (m *Metrics) switched(event SwitchEvent) {
	m.mu.Lock()
...
func (m *Metrics) gap() { m.mu.Lock(); m.snapshot.GapDetections++; m.mu.Unlock() }
