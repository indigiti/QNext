...
The current production path still has Upstox as the only configured live provider, so authority selection is operationally trivial today. The Q2 contract is intentionally generic so adding Dhan/Fyers/Angel/Zerodha or other providers does not change downstream canonical contracts.
