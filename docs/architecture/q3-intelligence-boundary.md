...
3. Feature snapshots only use bars whose event time is less than or equal to the prediction time.
...
7. Poor input quality produces a `WITHHELD` prediction state rather than silently acting on degraded data.
